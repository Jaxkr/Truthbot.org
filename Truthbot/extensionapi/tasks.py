...
def parse_article(url):
	...
	if (len(o.path) > 1 and o.path != '/'): #avoid parsing home pages
		...
		article.parse()
		article.nlp()
		new_article = ArticleModel(title=article.title, url=url, keywords=article.keywords)
		new_article.save()
	else:
		return
...
def recursive_create_parent(owner_obj, child_organization):
	if ('wikilink' in owner_obj):
		...
		if (infobox_table):
			...
			if not Organization.objects.filter(name=org_name).exists():
				new_organization = Organization(name=org_name, description=intro, url=web_address, wiki_url=url)
			...
			new_organization = Organization(name=org_name, description=intro, wiki_link=url)
			new_organization.save()
			new_organization.child_organizations.add(child_organization)
			new_organization.save()
